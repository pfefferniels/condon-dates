# The perforators of Stanford's red Welte rolls

What the perforations of Stanford's red Welte rolls say about the machines that cut them,
and about when. This is the corpus half of the study of the punch of WM 225; its first
half, the copies of 225 set against Hagmann's table, stays with the edition, in
`punch-225/README.md` in welte225.org. The two were parted on 26 September 2026, when the
scripts and the data of the corpus moved here from `punch-225`, so that every measurement
of these rolls is kept in one place, beside the readings of their dates in
`../data/readings.json`; how they came about is in the history of welte225.org. What they
allow an edition to conclude is stated as premises in `../docs/premises.jsonld`.
...
the copies of WM 225 measured in `punch-225/README.md` in welte225.org.
...
(`step.json`), on the dated copies whose period resolves:
...
## Looking for the dates (`../review/regions.py`, `../dates/`, `../review/`)
...
`../review/regions.py` is the first, geometric guess: the outer 900 px of either
...
## Two cautions about the catalogue
...
`corpus.py` writes the control and the corpus check, `pitch.py` and `step.py` the two
sweeps over every roll, `pitch.json` and `step.json`. The Stanford rolls are read over IIIF
at full resolution as PNG, so nothing measured has been through a lossy coder.
...
| `pitch.py` | the pitch of every roll, measured on the scan; writes `pitch.json` |
| `step.py` | the advance of every roll, from the analyses; writes `step.json` |
| `holes.py` | the perforations of a copy, from an analysis file or from an edition's IIIF regions |
...
| `pitch.json`, `step.json` | the two sweeps, one row per roll |

`pitch.py` and `step.py` take the index of the rolls from `../dates/rolls.py`. The study
of WM 225 in welte225.org uses these libraries from a checkout of this repository beside
it. Run them with `python3 corpus.py`, `python3 pitch.py` and `python3 step.py`; they need
`numpy`, `Pillow` and `requests` and the network. Downloads are kept in `../cache/`, which
is not committed.
